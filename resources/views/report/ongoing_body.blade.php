...
@section('content')
    ...
    <div class="alert alert-jim" id="inputText">
        <h2 class="page-header">Ongoing Documents</h2>
        <form class="form-inline" method="POST" action="{{ asset('documents/report/ongoingbody') }}" onsubmit="return searchDocument()">
            {{ csrf_field() }}
            <div class ="printhide">
               <div class="row">
                    <div class="col-md-6">
                        <input type="text" class="form-control" placeholder="Quick Search" name="keyword" autofocus>
                        <input type="hidden" class="form-control" placeholder="Quick Search" name="sec_id" value="{{$id}}">
                        <button type="submit" class="btn btn-info form-control" ><i class="fa fa-search"></i> Search</button>
                       
                        @if (count($on_keyword) > 0)
                        <a href="{{ asset('documents/report/ongoingbody/'.$id)}}" class="btn btn-info form-control">
                        View All</a>
                        @endif
